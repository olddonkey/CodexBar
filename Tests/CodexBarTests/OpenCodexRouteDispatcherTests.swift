...
struct OpenCodexRouteDispatcherTests {
    @Test(arguments: [
        ...
        ("xai", OpenCodexRouteTarget.subscription(.grok)),
        ("  XAI\n", OpenCodexRouteTarget.subscription(.grok)),
    ...
    @Test
    func `explicit xai model prefix routes to Grok`() {
        #expect(
            OpenCodexRouteDispatcher.route(modelName: "xai/grok-4.6") == .subscription(.grok))
        #expect(
            OpenCodexRouteDispatcher.route(
                provider: "openai",
                modelName: "  xai/grok-4.6  ") == .subscription(.grok))
    }
}
