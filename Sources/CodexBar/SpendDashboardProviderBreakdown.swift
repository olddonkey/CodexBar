            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(breakdown.displayName)
                        .font(.headline)
                    // A provider whose only source is folded into this header carries its disclosure here.
                    if !self.showsSubscriptionChildren(breakdown),
                       let disclaimer = breakdown.subscriptions.compactMap(\.costDisclaimer).first
                    {
                        Text(disclaimer)
                            .font(.caption2)
